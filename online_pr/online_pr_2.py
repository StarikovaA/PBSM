...
eeg_inlet = StreamInlet(eeg_streams_info[0],max_buflen=500, max_chunklen=1000000)
...
def epoch(raw,timestamp_raw,marker_timestamps,marker_id,marker,tmin,tmax,sampling_rate):
    ...
    print(np.where(marker_id == marker))
    time_points = marker_timestamps[np.where(marker_id == marker)[0]]
    indices = find_nearest_indices(time_points,timestamp_raw)
    epochs = []
    eeg_data = np.array(raw)
...
while(True):
    ...
    if STATE == States.ACQUISITION:
        ...
        if marker_sample[0] == "StartTask":
            ...
            while(receive_data_flag):
                ...
                eeg_chunk, eeg_chunk_timestamps = eeg_inlet.pull_chunk(max_samples=500, dest_obj=None)
                eeg_samples.append(eeg_chunk)
                eeg_timestamps.append(eeg_chunk_timestamps)
            
                # Receive data from marker stream
                marker_sample, marker_timestamp = marker_inlet.pull_sample()
                corrected_marker_timestamp = marker_timestamp
    ...
    if STATE == States.PROCESSING:
        ...
        epochs_S0 = epoch(eeg_samples_filter,eeg_timestamps,corrected_marker_timestamps,marker_samples,"S0",tmin,tmax,sampling_rate)
        epochs_S1 = epoch(eeg_samples_filter,eeg_timestamps,corrected_marker_timestamps,marker_samples,"S1",tmin,tmax,sampling_rate)
        epochs_S2 = epoch(eeg_samples_filter,eeg_timestamps,corrected_marker_timestamps,marker_samples,"S2",tmin,tmax,sampling_rate)
    ...
    if STATE == States.END:
        if marker_sample[0] == "G":
            ...
            break
# %%
print(np.shape(eeg_samples_filter))
print(np.shape(eeg_timestamps))
